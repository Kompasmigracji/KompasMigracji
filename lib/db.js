...
/* Пул підключень (замінив "нове з'єднання на кожен запит" — те створювало
   до 10с (connect) + 5с (query) затримки НА КОЖЕН q()-виклик, а маршрути з
   кількома послідовними запитами (напр. /api/subscribe: підписка + лід +
   задача) множили це в реальні 20-30+ секунд "зависання" на повільнішій
   мережі. Пул тримає з'єднання відкритими між викликами й перевикористовує
   їх — ті самі сигнатури q()/one(), нуль змін у 100+ файлах, що їх кличуть. */
function getPool() {
  if (!pool) {
    pool = new Pool({
      ...buildConfig(),
    ...
    // Без цього обробника помилка на простоюючому з'єднанні пулу (напр.
    // Supabase pooler розірвав неактивний конект) вилітає як unhandled
    // exception і валить весь serverless-процес.
    pool.on("error", (err) => {
      console.error("[db] Idle pool client error:", err?.message ?? String(err));
    });
  }
  return pool;
}
...
export async function q(text, params = []) {
  try {
    ...
    console.error("[db] Query error:", err?.message ?? String(err), "| sql:", text.slice(0, 120));
    throw err;
  }
}
