...
function VbIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" width="17" height="17">
      <path d="M11.398.002C9.218.044 4.9.44 2.895 2.294 1.507 3.678 1.011 5.677.955 8.119c-.056 2.44-.124 7.01 4.3 8.273h.004l-.004 1.892s-.03.81.505 1.005c.63.224 1.03-.424 1.65-1.103.34-.374.812-.921 1.165-1.33 3.212.27 5.676-.347 5.96-.436.644-.21 4.303-.677 4.9-5.527.617-4.99-.288-8.141-1.872-9.567l-.002-.002C16.068 1.874 14.066.376 10.398.206A10.69 10.69 0 0011 .213l.398-.01zm.054 1.636c.177 0 .352.004.523.011 3.077.14 4.72 1.334 5.15 1.738 1.296 1.172 2.062 3.939 1.53 8.18-.482 3.947-3.4 4.266-3.942 4.44-.24.078-2.445.626-5.287.436 0 0-2.097 2.527-2.75 3.18-.102.102-.219.143-.298.123-.11-.028-.14-.158-.139-.348l.018-3.098c-3.648-1.008-3.43-4.75-3.386-6.82.057-2.12.47-3.832 1.607-4.968 1.67-1.54 5.43-1.89 6.974-1.874zM12.05 4.975c-.337 0-.337.523 0 .528 2.393.018 4.383 1.28 4.388 4.72 0 .343.528.34.524 0-.006-3.74-2.22-5.22-4.912-5.248zm2.968 4.931c-.015.33.507.358.522.026.062-1.43-.83-2.69-2.51-2.797-.34-.022-.37.504-.027.526 1.38.083 2.067 1.092 2.015 2.245zM13.158 7.875c-.34-.012-.356.515-.016.526.834.03 1.288.567 1.264 1.395-.01.34.514.35.525.01.034-1.115-.618-1.9-1.773-1.931zm-.414 3.075c-.436-.244-.882-.1-1.064.207l-.38.648c-.173.295-.648.195-.648.195-2.92-.743-3.692-3.69-3.692-3.69s-.1-.475.195-.648l.648-.38c.307-.184.45-.628.207-1.064L6.828 4.49c-.248-.445-.83-.568-1.188-.272-.996.82-2.07 1.97-1.752 3.162.584 2.147 2.046 5.617 5.952 7.35 1.498.658 3.67 1.364 5.028.7 1.15-.558 2.12-1.7 2.858-2.762.263-.376.11-.918-.313-1.108l-2.67-1.61z"/>
    </svg>
  );
}

const SOCIAL = [
  { href: TG_LINK, icon: <TgIcon />, label: 'Telegram', color: '#229ED9' },
    ...
    <header className={`fixed top-0 left-0 right-0 z-50 bg-white transition-all duration-300 ${scrolled ? 'shadow-md' : 'border-b border-gray-100'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between h-[62px] gap-3">
        ...
        <Link to="/" className="flex items-center gap-2.5 no-underline flex-shrink-0">
          <img src={Logo} alt="logo" className="w-8 h-8 spin-slow" />
        ...
        <nav className="hidden md:flex items-center gap-1 flex-1 justify-center">
          ...
          <div ref={dropRef} className="relative">
            ...
            {dropOpen && (
              <div
                style={{ top: 'calc(100% + 6px)', minWidth: 240 }}
                className="absolute left-0 bg-white rounded-xl shadow-xl border border-gray-100 py-2 z-50"
              >
                {SERVICES.map((s, i) => (
                  <a
                    key={i}
          ...
          {[
            ...
            <a key={href} href={href} className="px-3 py-2 text-sm font-medium text-gray-600 hover:text-primary rounded-lg hover:bg-gray-50 transition-colors no-underline">
              {label}
            </a>
          ))}
        </nav>
          ...
          <div className="flex items-center gap-0.5 bg-gray-50 border border-gray-200 rounded-lg px-1.5 py-1">
            {['ua', 'pl', 'ru', 'en'].map((l) => (
              <button
                ...
                className={`px-1.5 py-0.5 text-xs font-bold rounded transition-all uppercase ${lang === l ? 'bg-primary text-white' : 'text-gray-400 hover:text-primary'}`}
              >
                {l}
              </button>
            ))}
        ...
        <div className="md:hidden bg-white border-t border-gray-100 px-4 py-4 flex flex-col gap-1">
          ...
          <div className="mt-3 flex items-center gap-2 px-3">
            ...
            <div className="flex gap-2 ml-auto">
              {SOCIAL.map(s => (
                <a key={s.label} href={s.href} target="_blank" rel="noreferrer" className="social-circle-btn" aria-label={s.label}>{s.icon}</a>
              ))}
            </div>
          </div>
        </div>
