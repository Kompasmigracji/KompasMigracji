...
export default function StarField() {
  ...
  useEffect(() => {
    ...
    let raf: number, t = 0, shootClock = 0;
    let shoot: { x: number; y: number; vx: number; vy: number; tail: number; life: number; decay: number } | null = null;
    ...
    const spawnShoot = () => {
      shoot = { x: c.width * 0.05 + Math.random() * c.width * 0.7, y: Math.random() * c.height * 0.45, vx: 7 + Math.random() * 6, vy: 3.5 + Math.random() * 3, tail: 140 + Math.random() * 90, life: 1, decay: 0.022 + Math.random() * 0.014 };
    };

    const draw = () => {
      t += 0.016; shootClock += 0.016;
      ...
      if (shoot) {
        ...
        const tx = shoot.x - Math.cos(angle) * len, ty = shoot.y - Math.sin(angle) * len;
        ...
        ctx.beginPath(); ctx.moveTo(shoot.x, shoot.y); ctx.lineTo(tx, ty);
        ctx.strokeStyle = grad; ctx.lineWidth = 1.8; ctx.stroke();
        shoot.x += shoot.vx; shoot.y += shoot.vy; shoot.life -= shoot.decay;
        if (shoot.life <= 0 || shoot.x > c.width + 80 || shoot.y > c.height + 80) shoot = null;
      }
  ...
  return (
    <canvas ref={ref} style={{ position: 'fixed', inset: 0, width: '100%', height: '100%', pointerEvents: 'none', zIndex: 0, animation: 'sf-fade 1s ease both' }} />
  );
}
