...
export default function PromoBanner() {
  ...
  const [step, setStep]       = useState<'promo' | 'pay'>('promo');
  ...
  return (
    <>
      ...
      <div onClick={close} style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(0,0,0,0.62)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16, animation: 'pb-backdrop 0.3s ease both' }}>
        <div onClick={e => e.stopPropagation()} style={{ background: '#0f172a', borderRadius: 22, padding: 'clamp(28px,5vw,44px) clamp(24px,5vw,40px)', maxWidth: 520, width: '100%', position: 'relative', border: '1px solid rgba(249,115,22,0.2)', fontFamily: "'Syne', sans-serif", animation: 'pb-card 0.38s cubic-bezier(0.22,1,0.36,1) both' }}>
          <button onClick={close} aria-label="Закрити" style={{ position: 'absolute', top: 14, right: 16, background: 'none', border: 'none', color: '#475569', fontSize: 22, cursor: 'pointer', lineHeight: 1, padding: 4 }} onMouseEnter={e => { e.currentTarget.style.color = '#fff'; }} onMouseLeave={e => { e.currentTarget.style.color = '#475569'; }}>✕</button>
            ...
            <>
              ...
              <div style={{ textAlign: 'center', marginBottom: 16 }}>
                <p style={{ fontSize: 11, color: '#334155', margin: '0 0 10px' }}>
                  ...
                  <Link href="/regulamin" onClick={close} style={{ color: '#64748b', textDecoration: 'underline' }}>Regulamin</Link>
                </p>
              </div>
              <Link href="/karta" onClick={close} style={{ display: 'block', textAlign: 'center', padding: '12px 0', borderRadius: 10, border: '1.5px solid #1e293b', color: '#475569', fontWeight: 600, fontSize: 13, textDecoration: 'none' }}>
                Дізнатися більше →
              </Link>
              <p style={{ fontSize: 10, color: '#1e293b', textAlign: 'center', margin: '14px 0 0', letterSpacing: '0.04em' }}>🛡️ ГАРАНТІЯ ПОВЕРНЕННЯ КОШТІВ ЗА 1 ДЕНЬ</p>
            </>
          )}
          ...
          {step === 'pay' && (
            <>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
                <button onClick={() => { setStep('promo'); setError(''); setEmail(''); }} style={{ background: 'none', border: 'none', color: '#64748b', cursor: 'pointer', fontSize: 13, fontFamily: 'inherit', padding: 0, display: 'flex', alignItems: 'center', gap: 5 }}>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="15 18 9 12 15 6"/></svg>Назад
                </button>
              ...
              <input
                type="email" value={email} onChange={e => setEmail(e.target.value)} onKeyDown={e => e.key === 'Enter' && pay()} placeholder="example@gmail.com" autoFocus
                style={{ width: '100%', padding: '11px 14px', borderRadius: 10, fontSize: 14, border: `1.5px solid ${error ? '#ef4444' : '#1e293b'}`, background: '#1e293b', color: '#fff', outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box', marginBottom: error ? 6 : 18 }}
              />
              ...
              <label style={{ display: 'flex', gap: 10, alignItems: 'flex-start', cursor: 'pointer', margin: '0 0 14px', userSelect: 'none' }}>
                <input type="checkbox" checked={agreed} onChange={e => setAgreed(e.target.checked)} style={{ marginTop: 3, accentColor: ORANGE, width: 16, height: 16, flexShrink: 0, cursor: 'pointer' }} />
              ...
              <button onClick={pay} disabled={loading || !agreed} style={{ width: '100%', padding: '13px 0', borderRadius: 10, border: 'none', cursor: loading || !agreed ? 'not-allowed' : 'pointer', background: loading || !agreed ? '#1e293b' : ORANGE, color: loading || !agreed ? '#475569' : '#fff', fontWeight: 700, fontSize: 14, fontFamily: 'inherit', transition: 'background 0.15s' }}>
