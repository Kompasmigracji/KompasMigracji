...
function StatusBadge({ status }: { status?: string | null }) {
  const s = (status && STATUS[status]) || STATUS.new;
  return (
    <span style={{
...
function StatusSelector({ lead, onUpdate }: { lead: Lead; onUpdate: (id: string | number, status: string) => void; }) {
  ...
  const change = async (newStatus: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        alert(json?.error || 'Помилка оновлення статусу');
      } else {
...
function LeadCard({ lead, onUpdate }: { lead: Lead; onUpdate: (id: string | number, status: string) => void; }) {
  const [expanded, setExpanded] = useState(false);
  const isTgLead = !!lead.chat_id;
  const name = (lead.first_name || lead.name || '—') as string;
  const urgColor = URGENCY_COLOR[(lead.urgency as string) || ''] || '#64748b';
...
export default function Admin() {
  const ADMIN_PASSWORD = process.env.NEXT_PUBLIC_ADMIN_PASSWORD || 'kompas2026';
  const [authed, setAuthed]   = useState<boolean>(typeof window !== 'undefined' && sessionStorage.getItem(ADMIN_KEY) === '1');
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const json = await res.json();
      if (!res.ok) {
        setError(json?.error || 'Помилка при завантаженні');
        setLeads([]);
      } else {
        setLeads(json?.data || []);
        setError(null);
