"use client";
/* /admin/workers — менеджер команди: таб "Команда" (CRUD) + таб "Канбан" (призначення справ). */
...
export default function WorkersPage() {
  ...
  const openAdd = () => {
    setForm({ role: "moderator", status: "active" });
    setFormErr(""); setTempPwd(""); setSelected(null);
    setModal("add");
  };

  const openEdit = (w) => {
    setForm({ full_name: w.full_name, email: w.email, role: w.role, status: w.status });
    setFormErr(""); setTempPwd(""); setSelected(w);
    setModal("edit");
  };
  ...
  return (
    <div>
      ...
      {modal === "delete" && (
        <DeleteModal
          worker={selected} saving={saving} error={formErr}
    ...
    <div className="kc-card" style={{ padding: "20px" }}>
      ...
      <div style={{ textAlign: "center", marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: 5, flexWrap: "wrap" }}>
          ...
          <span className={`kc-badge ${worker.status === "active" ? "kc-badge-green" : "kc-badge-red"}`}
            style={{ fontSize: 11 }}>
  ...
  return (
    <div className="kc-modal-bg">
      <div className="kc-modal" style={{ maxWidth: 380 }}>
        <div className="kc-modal-title" style={{ color: "var(--red)" }}>
          Видалити спеціаліста?
        </div>
        <p style={{ color: "var(--dim)", fontSize: 14, marginBottom: 16, lineHeight: 1.6 }}>
          ...
          Усі активні справи мають бути закриті або переназначені іншому спеціалісту.
        </p>
        {error && <div className="kc-error" style={{ marginBottom: 12 }}>{error}</div>}
        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
          <button className="kc-btn kc-btn-ghost" onClick={onClose} disabled={saving}>Скасувати</button>
...
/* ────────────────────────────────────────────
   KanbanBoard (Kanban tab)
   ──────────────────────────────────────────── */
function KanbanBoard({ workers, cases, filter, setFilter, assigning, setAssigning, busy, assign, dropRef, byStage, unassignedCount, totalActive }) {
  return (
    <div>
      {/* Worker filter tabs */}
                ...
                {stageCases.map(c => {
                  ...
                  return (
                    <div key={c.id} ref={isAssigning ? dropRef : null}
                      ...
                      {c.case_number && (
                        <div style={{ fontSize: 11, color: "var(--dim)", marginBottom: 6 }}>№ {c.case_number}</div>
                      )}
                        ...
                        <div style={{ marginTop: 8, border: "1px solid var(--border)", borderRadius: 8, overflow: "hidden" }}>
                          {c.assigned_to && (
                            <AssignOption label="Зняти призначення" labelColor="var(--red)"
                              onClick={() => assign(c.id, null)} disabled={busy === c.id} />
                          )}
                          {workers.map(w => (
                            <AssignOption key={w.id}
                              ...
                              onClick={() => assign(c.id, w.id)} disabled={busy === c.id} />
                          ))}
                        </div>
                      )}
                    </div>
