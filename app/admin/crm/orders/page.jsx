  ...
  return (
    <div className="flex flex-col h-full bg-[#f5f5f7] text-gray-800">
      ...
      <div className="bg-white/60 backdrop-blur-xl border-b border-black/10 px-8 py-5 flex items-center gap-6 sticky top-0 z-20">
        <h2 className="m-0 text-xl font-bold text-gray-900 tracking-tight">Заказы</h2>
      ...
      <div className="p-8 flex flex-col gap-6">
        ...
        {/* Filter Pills */}
        <div className="flex gap-2 overflow-x-auto pb-4 custom-scrollbar border-b border-black/5">
          {filters.map(f => (
            <button 
              key={f.id}
        ...
        <SpotlightCard className="bg-white/60 border border-black/10 rounded-2xl overflow-x-auto p-0">
          <table className="w-full min-w-[1200px] text-sm text-left">
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={order.id} className={`transition-colors hover:bg-white/60 border-black/5 ${index !== orders.length - 1 ? 'border-b' : ''}`}>
                  <td className="px-6 py-4"><input type="checkbox" className="accent-blue-500 cursor-pointer" /></td>
                  <td className="px-4 py-4 font-medium text-gray-800">{order.order_number}</td>
                  <td className="px-4 py-4">
                    <div className="flex items-center gap-2 text-purple-400">
                      <Icon name="phone" size={14} /> Приватний вайбер
                    </div>
                  </td>
                  <td className="px-4 py-4 text-gray-700">{new Date(order.created_at).toLocaleString('ru-RU')}</td>
                  <td className="px-4 py-4">
                  ...
                  <td className="px-4 py-4 text-gray-500">
                    <div className="flex items-center gap-2">
                      <Icon name="plus" size={12} className="cursor-pointer hover:text-gray-700" />
                      <Icon name="file-text" size={12} className="cursor-pointer hover:text-gray-700" />
                      <Icon name="edit-2" size={12} className="cursor-pointer hover:text-gray-700" />
                    </div>
                  </td>
                  <td className="px-4 py-4 text-gray-500">-</td>
                  <td className="px-4 py-4 text-gray-500">-</td>
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-3 text-gray-500">
                      ...
                      <Icon name="printer" size={16} className="cursor-pointer hover:text-gray-700 transition-colors" />
          ...
          {/* Pagination Footer */}
          <div className="p-4 flex justify-end items-center text-xs text-gray-500 border-t border-black/10 bg-black/20">
            <div className="flex items-center gap-6">
              <span>Показано 1 - {orders.length} из {orders.length} записей</span>
              <div className="flex items-center gap-2">
                <select className="bg-white/60 border border-black/10 text-gray-700 px-2 py-1 rounded-md outline-none focus:border-blue-500/50">
                  <option>10</option>
                  <option>20</option>
                  <option>50</option>
                </select>
